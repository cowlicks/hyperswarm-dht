...
impl QueryTable {
    ...
    pub fn get_peer(&self, peer: &udx::Peer) -> Option<udx::Peer> {
        self.peers
            .keys()
            .filter(|p| p.preimage().addr == peer.addr)
            .map(|p| udx::Peer::from(p.preimage().addr))
            .next()
    }

    pub fn get_token(&self, peer: &udx::Peer) -> Option<&Vec<u8>> {
        self.peers
            .iter()
    ...
    pub fn unverified_peers_iter(&self, parallelism: NonZeroUsize) -> FixedPeersIter {
        FixedPeersIter::new(
            self.peers
                ...
                .map(|(p, _)| udx::Peer::from(p.preimage().addr)),
            parallelism,
        )
    }
